...
Run this before benchmark_all_tools.py to verify all tools are installed,
binaries are present, environment variables are set, and the network is reachable.

Usage:
    python preflight.py                # check only
    python preflight.py --install      # check + install missing packages
    python preflight.py --smoke-test   # check + live-fire each tool (3 pages)
"""
...
def run_checks() -> tuple[dict[str, bool], bool]:
    ...
    core_deps = [
        ("markcrawl",      ["markcrawl.core"],  "pip install markcrawl"),
    ...
    check("markcrawl", ok, detail="" if ok else "→ pip install markcrawl")
...
def print_ready_status(tool_results: dict[str, bool]) -> None:
    ...
    if all_blocking_ok:
        ...
        print(f"    {bold('python benchmark_all_tools.py')}\n")
        print(dim("  Quick test (1 site, 1 iteration, skip warmup):"))
        print(dim("    python benchmark_all_tools.py --sites quotes-toscrape --iterations 1 --skip-warmup\n"))
    else:
        print(red("  Not ready — fix blocking issues above, then re-run:"))
        print(f"    {bold('python preflight.py --install')}\n")
...
def print_smoke_status(
    ...
    print(f"\n  {bold('python benchmark_all_tools.py')}\n")
