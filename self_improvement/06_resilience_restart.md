...
for f in runs/run_*/*/pages.jsonl; do
  python3 -c "
import json, sys
bad = 0
with open('$f') as fh:
...
grep -n "JSONDecodeError\|json.loads.*try\|load_jsonl" benchmark_*.py
```
...
grep -n "json.dump" benchmark_*.py
# Look for atomic write pattern (write to .tmp then rename)
grep -n "os.replace\|os.rename\|\.tmp" benchmark_*.py
```
...
```bash
for f in retrieval_checkpoints/*.json; do
  python3 -c "import json; json.load(open('$f'))" 2>/dev/null || echo "CORRUPT: $f"
done
for f in answer_quality_checkpoints/*.json; do
  python3 -c "import json; json.load(open('$f'))" 2>/dev/null || echo "CORRUPT: $f"
done
```
...
grep -n "open.*'w'" benchmark_quality.py
# General check
grep -n "open.*'w'" benchmark_*.py | grep -v "checkpoint\|report\|\.md"
```
...
```bash
# Check cache size
du -sh embed_cache/ 2>/dev/null || echo "No embed_cache"
# Count entries
ls embed_cache/ 2>/dev/null | wc -l
```

- [ ] Cache size is under 30 GB
- [ ] Is there an eviction mechanism? (`grep -n "evict\|cleanup\|purge" benchmark_retrieval.py`)
...
grep -n "tempfile\|mkdtemp\|TemporaryDirectory" benchmark_*.py
# Check for cleanup in finally blocks
grep -A3 "finally:" benchmark_*.py
```
...
grep -n "signal\|SIGINT\|KeyboardInterrupt" benchmark_*.py
```
...
python3 -c "
import json, glob
for f in glob.glob('retrieval_checkpoints/*.json'):
