...
grep -n "mem_limit\|NODE_OPTIONS\|NUQ_WORKER_COUNT\|WORKER_COUNT" \
  firecrawl/firecrawl-src/docker-compose.yaml \
  firecrawl/firecrawl-src/.env
```
...
```bash
cat firecrawl/firecrawl-src/.env
```
...
docker compose -f firecrawl/firecrawl-src/docker-compose.yaml ps
# Check for leftover jobs (if running)
docker compose -f firecrawl/firecrawl-src/docker-compose.yaml \
