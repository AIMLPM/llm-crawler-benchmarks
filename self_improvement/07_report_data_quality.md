...
for site_dir in runs/run_*/markcrawl/*/; do
  site=$(basename "$site_dir")
  echo ""
  echo "--- $site ---"
  for tool_dir in runs/run_*/*/"$site"/; do
...
with open('reports/SPEED_COMPARISON.md') as f:
    content = f.read()
...
python3 -c "
import json, glob
for f in sorted(glob.glob('retrieval_checkpoints/*.json')):
...
grep -n "Generated:" reports/*.md 2>/dev/null

echo ""
echo "=== Latest run directory ==="
ls -dt runs/run_*/ 2>/dev/null | head -1

echo ""
echo "=== Checkpoint ages vs source data ==="
latest_run=$(ls -dt runs/run_*/ 2>/dev/null | head -1)
if [ -n "$latest_run" ]; then
  echo "Run data: $(stat -f '%Sm' "$latest_run" 2>/dev/null || stat -c '%y' "$latest_run" 2>/dev/null)"
fi
ls -lt retrieval_checkpoints/ 2>/dev/null | head -5
```
...
Consider creating `check_data_quality.py` that runs all the
